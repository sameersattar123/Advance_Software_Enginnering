...
class Process:
    ...
    def set_response_ratio(self, time_passed):
        waiting_time = time_passed - self.arrival_time
        self.response_ratio = (waiting_time + self.burst_time) / self.burst_time
...
def sort_processes_according_to_arrival(process_list):
...
def execute_fcfs(processes):
    ready_queue = []
    time_passed = 0
    print('\nExecuting Processes according to First-Come, First-Served (FCFS).')
    while check_should_execution_proceed(processes):
        ...
        if len(ready_queue) == 0:
            ...
            ready_queue = sort_processes_according_to_arrival(ready_queue)
            ran_process = ready_queue[0]
...
# Existing SJF, SRTF, and HRRF functions...
...
if __name__ == "__main__":
    ...
    chosen_algorithm = input_entity(
        f'algorithm you want to execute (1 for FCFS, 2 for SJF, 3 for SRTF, 4 for HRRF)', 1, 4)

    if chosen_algorithm == 1:
        execute_fcfs(processes)
